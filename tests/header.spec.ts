...
const ОЖИДАЕМЫЕ = ['Куда поехать', 'Визы', 'Сборы', 'Статьи', 'Мои поездки'];
const ВНУТРИ = ['Все направления', 'По месяцам', 'Сезоны', 'Готовые маршруты', 'Сравнить',
...
test('3. ни один раздел не потерялся из навигации', async ({ page }) => {
  ...
  for (const надо of ['/countries/', '/visa/', '/seasons/', '/trips/', '/routes/', '/my/',
