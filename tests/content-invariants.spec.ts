...
test('Язык: в тронутой статье нет слов-паразитов, штампов и канцелярита', () => {
  ...
  const AD_TONE = [
    'жемчужина', 'затерянный в сердце', 'захватывает дух', 'захватывающий дух',
    'обязателен к посещению', 'обязательно к посещению', 'настоящий рай',
    'богатое наследие', 'яркая культура', 'незабываемые впечатления',
    'уникальная атмосфера', 'поражает воображение', 'сказочн',
  ];
  const root = join(DIST, '..');
  const touched = touchedPosts();
