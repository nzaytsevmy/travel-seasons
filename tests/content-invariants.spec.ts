...
// Плашка «Как написан этот пост» — ровно тот текст «откуда взято и когда сверено», который Никита запретил
// 08–10.09.2026. 11.09.2026 она стояла под строкой автора в 61 статье: «Это обзор по официальным источникам…
// сверил факты на актуальность», у тура в Японию — «цены сняты с витрин 9 сентября». Текст жил в шаблоне статьи
// и включался сам у каждой статьи-обзора (флажок в схеме по умолчанию true), поэтому чистка текстов статей
// его не видела. Оговорка обзора турфирмы («покупка не совершалась») остаётся отдельной строкой — её стережёт
// проверка «Брендовый обзор…» выше. Меряет собранную страницу: сам блок (элемент, а не правило в стилях — они лежат в каждой
// странице) и его слова. Пробелы в тексте бывают неразрывными — знаки заданы кодом, не литералом.
test('Откуда взято: в статьях нет плашки «Как написан этот пост»', () => {
  const PHRASES = ['Как написан этот пост', 'сверил факты на актуальность', 'Это обзор по официальным источникам'];
  const SPACES = new RegExp('[\\s' + String.fromCharCode(0xa0, 0x202f) + ']+', 'g');
  const JOINER = new RegExp(String.fromCharCode(0x2060), 'g');
  const bad: string[] = [];
  for (const f of files) {
    if (!f.includes('/blog/') || f.includes('/tag/')) continue;
    const html = readFileSync(f, 'utf8');
    const text = html.replace(/<(style|script)[\s\S]*?<\/\1>/g, ' ').replace(/<[^>]+>/g, ' ').replace(JOINER, '').replace(SPACES, ' ');
    const hit = /<[a-z]+\b[^>]*\bsource-disclaimer\b[^>]*>/.test(html) ? 'блок плашки' : PHRASES.find((p) => text.includes(p));
    if (hit) bad.push(`${f.replace(DIST, '')}: ${hit}`);
  }
  expect(bad, `плашка «откуда взято» в ${bad.length} статьях:\n${bad.slice(0, 20).join('\n')}`).toEqual([]);
});

// Хвост кода шаблона на странице. 11.09.2026 при снятии плашки «Как написан этот пост» вырезал блок до первой
// закрывающей скобки, а у блока внутри была своя развилка — под строкой автора во всех статьях осталось видимое «))».
// Проверка плашки его не видела (ни её слов, ни её блока), поймало сравнение скриншотов. Шапка статьи — от начала
// статьи до текста — собирается шаблоном целиком: скобкам кода и «&&» в её видимом тексте взяться неоткуда.
test('Шаблон статьи: в шапке нет видимых кусков кода', () => {
  const bad: string[] = [];
  for (const f of files) {
    if (!f.includes('/blog/') || f.includes('/tag/')) continue;
    const html = readFileSync(f, 'utf8').replace(/<(style|script)[\s\S]*?<\/\1>/g, ' ');
    const head = html.match(/<article\b[^>]*\bpost-content\b[^>]*>([\s\S]*?)<div\b[^>]*\bprose\b/);
    if (!head) continue;
    const text = head[1].replace(/<[^>]+>/g, ' ');
    const junk = text.match(/[{}]|\)\)|&&/);
    if (junk) bad.push(`${f.replace(DIST, '')}: «${junk[0]}»`);
  }
  expect(bad, `куски кода в шапке ${bad.length} статей:\n${bad.slice(0, 20).join('\n')}`).toEqual([]);
});
...
test('Заголовок: слова не слипаются через границу тега', () => {
  ...
  for (const файл of files) {
    ...
    for (const [, внутри] of html.matchAll(/<h1[^>]*>([\s\S]*?)<\/h1>/gi)) {
      ...
      if (стыки.length) {
        // Теги снимаем повтором до неподвижной точки: одиночная замена по шаблону тега
        // оставляет вложенные остатки (находка проверки кода, js/incomplete-multi-character-sanitization).
        let текст = безПереносов;
        for (let было = ''; было !== текст;) { было = текст; текст = текст.replace(/<[^>]*>/g, ''); }
