...
test('Заголовок: слова не слипаются через границу тега', () => {
  ...
  for (const файл of files) {
    ...
    for (const [, внутри] of html.matchAll(/<h1[^>]*>([\s\S]*?)<\/h1>/gi)) {
      ...
      if (стыки.length) {
        // Своё вырезание тегов не пишем: у `видимыйТекст` в шапке разобрано,
        // почему наивное /<[^>]+>/ мерит не то (закрывающий тег с мусором,
        // «>» внутри атрибута). Это же выражение отметил CodeQL.
        // Пустая замена вместо пробела: иначе сообщение об ошибке само склеит
        // разорванное и покажет «Абхазия в апреле» там, где на странице
        // напечатано «Абхазияв апреле».
        const текст = видимыйТекст(безПереносов, '').replace(/[^\S\n]+/g, ' ').trim();
