  ...
  for (const d of DIRECTIONS) {
    ...
    for (const f of [
      ...
      const t = readFileSync(f, 'utf8')
        // ⛔ Вырезаем служебное надёжно, иначе текст скрипта считается видимым текстом
        //    страницы. Сканер кода поймал два промаха разом: без флага регистра мимо идёт
        //    <SCRIPT> заглавными, а без \b и пробела в закрывающем теге — <script foo> и
        //    </script >. Комментарии убираем первыми: внутри них бывает разметка.
        .replace(/<!--[\s\S]*?-->/g, ' ')
