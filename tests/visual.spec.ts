...
for (const page of PAGES) {
  test(`${page.name} — visual`, async ({ page: pwPage }) => {
    ...
    // ⛔ Шрифт бренда подключён с font-display:optional: не успел файл к ПЕРВОЙ раскладке —
    //    движок рисует страницу запасным шрифтом до конца её жизни и потом НЕ подменяет (так
    //    выбрано ради нулевого сдвига вёрстки). Значит одна и та же сборка даёт два законных
    //    вида, и fullPage-снимок краснеет через раз: 12.09.2026 реестр виз на телефоне выходил
    //    то 25 605, то 25 625 px высотой. Колонки сетки карточек считаются по самому длинному
    //    слову, а у запасного шрифта оно другой ширины — отсюда и разъезд. Повторный заход
    //    берёт шрифт из кеша и всегда укладывает текст брендовым: это вид, который видят
    //    читатели на любой странице после первой. Допуск и fullPage не тронуты.
