...
// Подходят ли российские вилки. У нас розетки C и F; розетка E принимает вилку C и гибридную E/F, какие
// продают в России. Все типы страны из этих — 'все', часть — 'часть', ни одного — 'нет'. Одна проверка на
// весь сайт: страница страны и чек-листы сборов не должны спорить (11.09.2026 разошлись у 26 стран: сборы
// писали «переходник НЕ нужен», если подходил хоть один тип, — например, у Аргентины с розетками C/I).
const RU_FIT = new Set(['C', 'E', 'F']);
export function plugFit(types) {
  const list = String(types || '').split('/').map((t) => t.trim()).filter(Boolean);
  const fit = list.filter((t) => RU_FIT.has(t)).length;
  return list.length && fit === list.length ? 'все' : fit > 0 ? 'часть' : 'нет';
}

// Подсказка для чек-листов сборов — нужен ли универсальный переходник.
export function plugAdvice(plug) {
  if (!plug) return 'Уточнить тип розетки перед поездкой.';
  const fit = plugFit(plug.types);
  if (fit === 'все') return 'Российские вилки Type C/F подходят — переходник НЕ нужен.';
  if (fit === 'часть') return `Российские вилки подходят не ко всем розеткам (Type ${plug.types}) — переходник пригодится. Купи универсальный (3–5 типов) — окупится за 1 поездку.`;
...
// Коротко для строки «Розетка» и карточки на странице страны — та же проверка, что у чек-листов сборов.
export function plugVerdict(types) {
  const fit = plugFit(types);
  if (fit === 'все') return { short: 'переходник не нужен', long: 'Вилки как в России — переходник не нужен.' };
  if (fit === 'часть') return { short: 'переходник пригодится', long: 'Российские вилки подходят не ко всем розеткам — переходник пригодится.' };
  return { short: 'нужен переходник', long: 'Российские вилки не подходят — нужен переходник.' };
}
