...
//   docs      — [] = показывать везде; иначе режим документов направления:
//               'ru' (регион России), 'id' (пускают по внутреннему паспорту РФ:
//               Абхазия, Армения, Казахстан, Киргизия), 'abroad' (нужен загран).
//               Режим задаёт страница, которая рисует чек-лист; без него список
//               общий и показывает всё — так он и работает вне страниц направлений.
...
export const ITEMS = [
  // ─── Документы и деньги ───
  F('passport-int', 'Загранпаспорт', 'docs', { docs: ['abroad'], note: 'срок действия 6+ месяцев на дату въезда' }),
  F('passport-int-kid', 'Загранпаспорт ребёнку', 'docs', { docs: ['id'], who: ['kids'], note: 'взрослым хватит внутреннего; свидетельство о рождении для выезда не принимают с 20 января 2026 — кроме Абхазии, там его принимают до конца 2027 года' }),
  ...
  F('visa', 'Виза или разрешение на въезд', 'docs', { docs: ['abroad'], note: 'если требуется — проверить заранее на сайте консульства' }),
  F('insurance', 'Медицинская страховка', 'docs', { note: 'за границей часто обязательна; для визы — нужна' }),
  F('oms', 'Полис ОМС', 'docs', { docs: ['ru'], note: 'по России работает; за границей не действует' }),
