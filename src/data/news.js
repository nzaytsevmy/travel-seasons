...
// ⛔ Хвост снимаем перебором последних слов, а не одним выражением: выражение
// с повторением внутри повторения анализ кода (CodeQL, js/redos) пометил как
// экспоненциальное — на строке «таб-цифра» × 26 оно думало 2,9 секунды.
const DANGLING_WORD = /^(\d+|[а-яё]{1,2}|для|при|про|без|над|под|через|после|перед|между|около|что|как|или|чем|где|когда|если)$/i;
function clipWords(s, max) {
  const cut = s.slice(0, max);
  const words = (s[max] === ' ' ? cut : cut.replace(/\s+\S*$/, '')).split(/\s+/);
  while (words.length > 1 && DANGLING_WORD.test(words.at(-1).replace(/[,:;—–-]+$/, ''))) words.pop();
