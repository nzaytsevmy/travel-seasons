...
def alert_mode(c) -> None:
    ...
    # Чекпоинты дорожной карты — В ДЕНЬ, на который назначены. Недельный отчёт
    # ходит только по понедельникам, и напоминание, поставленное на среду,
    # доезжало бы на пять дней позже срока. Здесь сравнение строгое (== TODAY),
    # иначе просроченный пункт сыпался бы каждый день до отметки «сделано»;
    # роль подстраховки играет понедельничный отчёт, он берёт всё, что <= TODAY.
    for r in c.get("roadmap", []):
        if not r.get("done") and str(r.get("date", "")) == str(TODAY):
            alerts.append(f"📍 Сегодня по плану: {r.get('label','')}\n▶ {r.get('prompt','')}")
...
def weekly_mode(c) -> None:
    ...
    if tp["ok"]:
        ...
        if tp["att_share"] < 60:
            ...
            # каждой ссылки; метка стоит ДО адреса назначения). Сплошная проверка
            # собранного сайта в тот же день: 7995 ссылок партнёрки — все помечены,
            # все с меткой в правильном месте, ни одного нарушения. То есть
            # обезличенные клики приходят НЕ со страниц сайта, и старая подсказка
            # уводила искать несуществующую поломку.
            L.append(f"   ⚠️ атрибуция {tp['att_share']}% — часть кликов без метки. "
                     f"На страницах сайта метка есть у всех ссылок (стерегут тесты) — "
                     f"значит источник вне сайта: ссылки в канале или обходы роботов")
    else:
