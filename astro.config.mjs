...
export default defineConfig({
  ...
  integrations: [
    ...
    // -5-15% размер страниц.
    //
    // ⛔ Минификация HTML — самая дорогая часть сборки: 2351 страница, 4,3 минуты
    // из семи (замерено по журналу проверки 01.08.2026). На выкладке она нужна,
    // а проверкам — нет: минификация не меняет ни отрисовку страницы, ни разметку
    // для поисковика, а раньше эти же 4 минуты платились ещё трижды за прогон.
    // Поэтому в проверках она выключается переменной, а на прод уезжает как было.
    compress({
      CSS: true,
      HTML: process.env.SKIP_HTML_MIN !== '1',
      JavaScript: true,
      SVG: true,
