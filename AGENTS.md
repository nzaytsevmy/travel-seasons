...
- **Visual/functional тесты:** `npm run check:visual` (Playwright). Неочевидное:
  ...
  - ⚠️ **Скриншотные эталоны есть ТОЛЬКО под darwin** (116 файлов `*-darwin.png`, линуксовых нет).
    Поэтому CI-джоба `visual` их не сравнивает — она запускается с `--grep-invert "— visual"`
    и гоняет 98 неплатформозависимых тестов, исключая 29 скриншотных. **Зелёная джоба
    «Visual Regression» НЕ означает, что вёрстка не поехала** — она скриншоты не смотрит.
  - Отсюда: на linux-VM `npm run check:visual` либо сам напишет отсутствующие linux-эталоны,
    либо упадёт на первом прогоне — в обоих случаях это НЕ проверка вёрстки. Единственная
    настоящая скриншотная проверка — локальный прогон на маке против darwin-эталонов.
  - Из darwin-эталонов на 17.07.2026 протухли три (`blog-more-checklist`, `blog-kuda-more-deti`,
    `packing-egypt-june` — дельта −1200/−1032px). Разбирается отдельной задачей; вслепую
    их не перерисовывать.
