...
export function boldLeads(src) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // ⛔ Перечень — не ритуал (решение Никиты 14.08.2026). В обзоре двадцати
    // направлений абзацы идут подписями: «**ОАЭ, Абу-Даби.** …», «**Черногория.** …».
    // Первая такая подпись выглядит как жирный лид, хотя это элемент списка, и
    // гейт краснел на здоровом тексте — автор дописывал подводки только чтобы
    // его успокоить. Считаем ритуалом лишь одиночный жирный зачин: если до
    // следующего заголовка есть ещё хотя бы два таких же, это перечень.
    let siblings = 0;
    for (let k = j + 1; k < lines.length && !/^##\s/.test(lines[k]); k++) {
      if (lines[k].trim().startsWith('**')) siblings++;
    }
