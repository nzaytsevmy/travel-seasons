...
// Повторяем замену, пока строка меняется: одиночный проход по «<scr<script>ipt>»
// склеивает половинки обратно в живой тег. Каждый проход строку только укорачивает,
// поэтому цикл конечен.
function доПустого(s, re) {
  let было;
  do { было = s; s = s.replace(re, ''); } while (s !== было);
  return s;
}
...
function stripMdx(body) {
  ...
  t = доПустого(t, /<[A-Z][A-Za-z0-9]*\b[\s\S]*?\/>/g);
  // встроенные картинки и стили — это графика, а не проза: подписи осей и
  // обрывки путей в текстовом файле для ботов только шумят. Срезаем целиком.
  t = доПустого(t, /<(svg|style|script)\b[\s\S]*?<\/\1>/gi);
  // инлайновые ссылки (в т.ч. партнёрские с href={переменная}) — оставить текст
  let было;
  do { было = t; t = t.replace(/<a\b[^>]*>([\s\S]*?)<\/a>/gi, '$1'); } while (t !== было);
  // остальная вёрстка, написанная в статье руками (<p>, <span class="cta-note">,
  // <div class="editorial-cta">): текст внутри нужен, теги и имена классов — нет.
  // Требуем после имени тега пробел, / или > — иначе съело бы автоссылку <https://…>.
  t = доПустого(t, /<\/?[a-z][a-z0-9]*(?=[\s/>])[^>]*>/gi);
  return t
    ...
    // После снятия HTML-тегов перед переносом иногда остаётся пробел. Он не
    // виден читателю, но создаёт шум в каждом сгенерированном diff.
...
- [Новости путешествий](${SITE}/novosti/): ежедневная лента — изменения въездных правил, природа и нацпарки, доступ к местам. Каждая заметка с датой события, датой проверки и ссылкой на первоисточник
- [Изменения правил въезда](${SITE}/izmeneniya/): хронология визовых изменений со статусом «действует» / «принято, не вступило» и первоисточником у каждого пункта
...
- [Готовые маршруты](${SITE}/routes/): планы по дням из проверенных гайдов; даты, состав и остановки можно менять до входа
