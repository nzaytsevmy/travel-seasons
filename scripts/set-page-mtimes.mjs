...
// ⛔ Заглушки-перенаправления (старые адреса из astro.config) — отдельный
//    случай, найден 29.08.2026: путь «mexico_old» подходит под шаблон страницы
//    страны, и 177 заглушек унаследовали дату чужой правки этого шаблона —
//    на час попали в «свежее» окно гейта дат. Их настоящий источник — конфиг
//    перенаправлений: дата его последней правки и есть дата заглушки.
const датаКонфига = датаФайла(['astro.config.mjs']);
const этоЗаглушка = (файл) => {
  try { return readFileSync(файл, 'utf8').includes('http-equiv="refresh"'); }
  catch { return false; }
};
