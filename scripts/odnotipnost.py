...
def month_specific_facts(path):
    ...
    без данных. Настоящий маркер измеренного факта — строка «Замер по <город>», её
    печатает только реальный погодный блок (строка источника снята 10.09.2026)."""
    t = text(path)
    facts = 0
    # реальный погодный блок: замер по конкретному городу
    facts += len(re.findall(r'Замер по\s+\S+', t))
