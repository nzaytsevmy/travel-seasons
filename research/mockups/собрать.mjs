...
function apply(){var n=0;
 ...
 // ⛔ Сводка собирается УЗЛАМИ, без подстановки в разметку. В неё входит
 // строка из поля поиска: через innerHTML введённое человеком исполнилось бы
 // как разметка (нашёл CodeQL на всех трёх макетах).
 sum.textContent=''; sum.appendChild(document.createTextNode('Показано '));
 var жир=document.createElement('b'); жир.textContent=String(n); sum.appendChild(жир);
 sum.appendChild(document.createTextNode(' из 74 · '+(ч.length?'отбор: '+ч.join(' · '):'отбор не задан')));}
