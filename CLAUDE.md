...
3. **CWV (Lighthouse mobile)** на изменённых типах: **CLS < 0.1** (цель 0.000), LCP < 2.5s, TBT≈0.
   ...
   +10 мин. Плюс на ЗАЯВКЕ страница открывается ОДИН раз вместо трёх: там нужен
   ответ «не сломал ли я скорость», а не точное число. По понедельникам и при
   ручном запуске остаётся 3 захода с медианой — это и есть настоящий замер,
   на него и опираться в отчётах. Одиночный заход на общем раннере шумит.
   ⛔ Если заявка падает по скорости — не «чинить» её вслепую: сначала повторить
   замер тремя заходами через `workflow_dispatch`, иначе можно править шум. Правки кода, вёрстки, стилей и настроек сборки проверяются как раньше;
