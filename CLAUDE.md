...
6. **Playwright-регресс** зелёный. Эталоны устаревают — если фейлы на нетронутых страницах,
   ...
   ⚡ В CI набор разделён на два прогона, и сливать их обратно НЕЛЬЗЯ:
   «Visual Regression» гоняет ТОЛЬКО пиксельное сравнение (120 тестов) и имеет
   `paths-ignore: src/content/**` — под эталоном 14 канареек, текста среди них нет.
   «Content Gate» гоняет ВСЁ остальное (580 тестов: инварианты контента, ритм,
   гейт новостей, чек-лист, калькулятор) на КАЖДОЙ заявке, без фильтра.
   Причина деления: 03.08.2026 весь набор стоял в визуальной джобе, и фильтр
   уносил с собой инварианты — гейт для контента не запускался на правках
   контента. Проверено на живой заявке: изменено 4 статьи, тестов запущено 0.
   Деление сверять подсчётом: 700 = 120 + 580.
