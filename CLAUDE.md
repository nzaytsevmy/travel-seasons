...
3. **CWV (Lighthouse mobile)** на изменённых типах: **CLS < 0.1** (цель 0.000), LCP < 2.5s, TBT≈0.
   ...
   +10 мин. Плюс на ЗАЯВКЕ ДЕСКТОПНЫЙ замер делает один заход вместо трёх.
   ⛔ МОБИЛЬНЫЙ — всегда 3 захода с медианой, снижать нельзя. Проверено
   03.08.2026: на одном заходе мобильный упал на 3 шардах из 4 («performance
   ниже 0.85»), хотя правка трогала только настройки CI. Перезамер тремя
   заходами на том же коммите — все 8 прогонов зелёные, то есть это был шум.
   Мобильная оценка идёт близко к порогу, median ей нужен; у десктопной запас
   больше, и на одном заходе она прошла все четыре шарда.
   ⛔ Если замер падает — не «чинить» вслепую и НЕ двигать порог (порог
   маскирует регресс): сначала повторить тремя заходами через `workflow_dispatch`.
   Начнёт мигать десктоп — вернуть ему 3 захода. Правки кода, вёрстки, стилей и настроек сборки проверяются как раньше;
